    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        <p className="text-sm text-red-500 mt-2">
          Make sure the API is running: <code className="bg-red-100 px-1 rounded">atmosphere start</code>
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-gray-500 text-sm ml-2">({agent.agent_id})</span>
                    </div>
                    <span
            ...
            <div className="space-y-2">
              {activity.map((item, i) => (
                <div key={i} className="text-sm border-b pb-2">
                  ...
                  {item.agent_id && (
                    <span className="text-gray-500">Agent: {item.agent_id}</span>
                  )}
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-4 text-center">
          ...
          <div className="text-gray-500">Recent Events</div>
        </div>
        <div className="bg-white rounded-lg shadow p-4 text-center">
          <div className={`text-3xl font-bold ${connected ? "text-green-600" : "text-red-600"}`}>
            {connected ? "●" : "○"}
          </div>
          <div className="text-gray-500">{connected ? "Live" : "Offline"}</div>
        </div>
      </div>
